...
def test_grade_session_parses_json_and_computes_score():
    ...
    with patch("backend.services.claude_grader.generate_text", return_value=payload):
...
def test_grade_session_invalid_json_raises():
    with patch("backend.services.claude_grader.generate_text", return_value="not json at all"):
        import pytest
